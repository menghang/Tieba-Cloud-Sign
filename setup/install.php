...
if (!isset($_GET["step"]) || $_GET["step"] == 0) {echo "<meta http-equiv=\"refresh\" content=\"0;url=install.php?step=2\">";exit;}
...
define(\'DB_PREFIX\',\'tc_\');
  ...
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 30%">
    <span class="sr-only">30%</span>
				...
				echo '<div class="input-group"><span class="input-group-addon">自动获得数据库配置信息</span><select name="from_config" class="form-control"  onchange="if(this.value == \'0\') { $(\'#db_config\').show(); } else { $(\'#db_config\').hide(); }"><option value="0">否</option><option value="1" selected="true">是</option></select></div><br/>';
...
echo "<script type=\"text/JavaScript\">$(\"#db_config\").hide();</script>";
...
define(\'SYSTEM_SALT\',\'\');';
  ...
  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="40" aria-valuemin="0" aria-valuemax="100" style="width: 90%">
    <span class="sr-only">90%</span>
				...
				echo '恭喜你，安装已经完成<br/><br/>云签到的安装已经全部完成，开始使用吧~</br>如需上传插件或管理文件，请使用Plus分支安装<br/><br/><br/>为保证站点安全，系统已在 /setup 文件夹下放置了 install.lock 文件，如果您的服务器不支持写入，请手动放置一个空的 install.lock 文件到此文件夹下，否则任何人都有权限重新安装您的云签到。<br/><br/><b>请您尊重作者，无论如何都不要删减云签到的版权</b><br/><br/><input type="button" onclick="location = \'../index.php\'" class="btn btn-success" value="进入我的云签到 >>">';
